...
export class DifferableList<T> {
  ...
  static fromArray<T>(arr: T[]): DifferableList<T> {
    return new DifferableList<T>(List<T>(arr));
  }
  ...
  unshift(data: T) {
    ...
    return new DifferableList<T>(this.data.unshift(data));
  }
  ...
  pop(): DifferableList<T> {
    ...
    return new DifferableList<T>(this.data.pop());
  }
  ...
  set(idx: number, item: T) {
    ...
    return new DifferableList<T>(this.data.set(idx, item));
  }
  ...
  splice(idx: number, len: number, ...values: any[]): DifferableList<T> {
    ...
    for (let i = len; i < this.data.size; i += 1) {
      this.changes.push({
        currentIndex: values.length + i,
        previousIndex: i,
        item: this.data.get(i),
        trackById: trackByIdentity
      });
    }
    return new DifferableList<T>(this.data.splice(idx, len, ...values) as List<T>);
  }
